...
class Trainer():
    ...
    def train(self,config,criterion,optimizer,scheduler=None,epochs=1000,batch_size=1024,
              loss_plot=True,progress=False):
        self.model = Seq2Seq(config).to(DEVICE)
    ...
    def inference(self):
        ...
        self.predicts = []  
        with torch.no_grad():      
            for v in self.test_batch:
                input_tensor = torch.tensor(v[0],device=DEVICE,dtype=torch.long).t()
                _, _, todec = self.model.encoder(input_tensor,inference=True) 
                ...
                dec_hid = todec #[B, Hsum]

                batch_tmp = torch.zeros(1,dec_input_tensor.size(1),dtype=torch.long,device=DEVICE)
                for i in range(input_tensor.size(0)): # teacher forcing
                    dec_o, dec_hid = self.model.decoder(dec_input_tensor,dec_hid,inference=inference)
    ...
    def evaluate(self):
        ...
        for s,t in zip(self.test_batch,self.predicts):
            for inp, out, pred in zip(s[0],s[1],t.T): # [B, T]
                ...
                p = [self.id2sm[j.item()] for j in pred]
        ...
        self.accuracy = len(self.pred_df.query("judge == True")) / len(self.pred_df)
    ...
    def load(self,config,path="trained_params.pth"):
        self.model = Seq2Seq(config).to(DEVICE)
...
class Generator():
    ...
    def generate(self,state,maxlen=200,batch=1024):
        self.model.eval()
        with torch.no_grad():
            self.generates = []
            ...
            for s in state:
                dec_hid = self.model.encoder.linear2(s[0])
    ...
    def get_descriptor(self,smiles,batch=1024):
        ...
        with torch.no_grad():
            for v in temp:
                ...
                _, desc, _ = self.model.encoder(t,inference=True)
                result.append(desc.cpu().detach().numpy())
